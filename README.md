...
### 0. 完成形
今回はフロントエンドの演習です。  
ラーメンを追加するページ`post.html`が元々あったと思いますが、以下の画像を参考にデザインを限りなく近づけてください。

![image](https://github.com/user-attachments/assets/5bd8241c-d3ff-4a97-8d6f-5b2a34958b91)

### 1. HTMLに関する条件・ヒント
- 商品名・店名・評価の入力欄を縦に並べてください
- 評価に関しては、★を5つ並べて、その中から選択できるようにしてください

### 2. CSSに関する条件・ヒント
- 投稿ボタンの背景色`#007BFF` マウスオーバー時の背景色`#0056b3`角丸5px
- リンク(トップに戻る)の文字色`#007BFF`
- 評価の星は通常時`#ccc`、マウスオーバー時や選択時は`gold`

### 3. JavaScriptに関する条件・ヒント
- 評価の星をクリックすると、その星までの評価が選択されるようにしてください(図は3つ目の星が選択されている状態)
- 星をマウスオーバーした際も、同様にその星までの色が変わるようにしてください
- 選択された★は`id`をもとに持ってきても構いませんが、余裕があれば`data-*`属性について調べて使ってみるのがおすすめです
  (そのためには`<span class="eval-star" data-value="1">★</span>`のようにhtmlを書いておく必要があります。)
